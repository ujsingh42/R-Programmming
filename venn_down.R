...
df <- read.csv("down.csv", stringsAsFactors = FALSE)
...
venn_data <- data.frame(
  "siMECOM KD" = c(only_exp9, rep(NA, max_len - length(only_exp9))),
...
write.csv(venn_data, file = "venn_down_diagram_results.csv", row.names = FALSE, na = "NA")

# Create a Venn diagram with adjusted text size and increased background margins
venn.plot <- venn.diagram(
  x = list("siMECOM KD" = set_exp9, "JIB04 treatment" = set_exp10),
...
png("JIB04 treatment vs siMECOM_KD.png", width = 1600, height = 1600, res = 100)  # Increased canvas size
par(mar = c(6, 6, 6, 6))  # Adjust margins: bottom, left, top, right
grid.draw(venn.plot)
dev.off()

# Save the Venn diagram to a PDF file with larger background margins
pdf("JIB04 treatment vs siMECOM_KD.pdf", width = 16, height = 16)  # Larger canvas
par(mar = c(6, 6, 6, 6))  # Adjust margins: bottom, left, top, right
grid.draw(venn.plot)
dev.off()
